...
//	std::cout<<"Start:"<<std::endl;
    for (size_t i = 0; i < 100 && percent_N(seq) > 0.05; i++) {
        ...
        auto dupr = repeat_region[(*chr).first];
...
bool is_overlapping(position curr, std::vector<struct_var> svs) {
    ...
    for (size_t i = 0; i < svs.size(); i++) {
        if (strcmp(svs[i].pos.chr.c_str(), curr.chr.c_str()) == 0) {
            if (svs[i].pos.stop + 5000 >= curr.start && svs[i].pos.start <= curr.stop + 5000) {
                return true;
            }
        }
    }
...
std::vector<struct_var> generate_mutations(std::string parameter_file, std::map<std::string, std::string> genome, std::unordered_map<std::string, std::vector<dupregion>> repeate_result) {
...
void apply_mutations(std::map<std::string, std::string> &genome,std::map<std::string, std::string> &org_genome, std::vector<struct_var>& svs,int max_dup_amp, bool if_recom) {
    srand(time(NULL));
...
void write_fasta(std::string output_prefix, std::map<std::string, std::string> genome, bool append, std::string suffix) {
...
void write_sv(std::string output_prefix, std::vector<struct_var> svs,bool append) {
    std::string out = output_prefix;
    out += ".bed";
    FILE *file2;
    if (append) {
...
std::string print_vcf_sv(std::string chr, int pos, int type_id, std::string end_chr, int end_pos, int id, std::string seq, std::string ref, std::string gt,int dup_amp, bool recom) {
    std::string type = "";
    switch (type_id) {
        case 0:
            type = "DUP";
...
void print_vcf_svs(FILE *& file, std::vector<struct_var> svs, int id) {
    for (size_t i = 0; i < svs.size(); i++) {
        //write pseudo bed:
        if (svs[i].type == 3) {
            fprintf(file, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].target.chr, svs[i].target.start, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
            fprintf(file, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.stop, svs[i].type, svs[i].target.chr, svs[i].target.stop, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
        } else { // all other types:

            fprintf(file, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].pos.chr, svs[i].pos.stop, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
        }
        id++;
    }
}
...
std::unordered_map<std::string, std::vector<dupregion>> parser_repeate(std::string repeate_region) {
    std::unordered_map<std::string, std::vector<dupregion>> result;
    std::ifstream infile(repeate_region);
    int start,end;
    std::string line;
...
void simulate_SV(std::string ref_file, std::string parameter_file, float snp_freq, bool coordinates, std::string output_prefix, std::string repeat_region) {
    ...
    srand(time(NULL));
    parameter par = parse_param(parameter_file);
    auto repeate_result = parser_repeate(repeat_region);
    ...
    std::map<std::string, std::string> genome2;
    std::map<std::string, std::string> ori_genome = read_fasta(ref_file, min_chr_len);
    ...
    if (coordinates) {	//simulate reads
        svs = generate_mutations(parameter_file, genome, repeate_result); // select coordinates and mark genome.
        if (!par.diploid) {
        ...
        for (auto &item: genome) {
            genome2[item.first] = item.second;
        }
        volatile auto t = genome["chr14"] == genome2["chr14"];
        apply_mutations(genome,ori_genome, svs,par.dup_max_amp,false);	// apply changes to the genome (e.g. insertions).
        apply_mutations(genome2,ori_genome, svs,par.dup_max_amp, true);	// apply changes to the genome (e.g. insertions).
        volatile auto t2 = genome["chr14"] == genome2["chr14"];
        volatile int t3 = 9;
    ...
    /// Start writing out:
    std::cout << "write genome1" << std::endl;
    write_fasta(output_prefix, genome, false, "_f");
    std::cout << "write genome recom" << std::endl;
    write_fasta(output_prefix, genome2, true, "_frecom");
    ...
    if (par.diploid) {
        ...
        svs2 = generate_mutations(parameter_file, genome, repeate_result); // select coordinates and mark genome.

        //TODO swap a few svs vs. svs2!

        for (size_t i = 0; i < svs2.size(); i++) {
            float x = ((float) rand() / (float) (RAND_MAX));
            if (x < (par.hom_rate) && (!svs[i].recom && !svs2[i].recom)) { // homozygous SV
                svs2[i] = svs[i];
                //svs[i].gt = "1/1";
                if (svs[i].type == 3) {
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].target.chr, svs[i].target.start, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.stop, svs[i].type, svs[i].target.chr, svs[i].target.stop, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
                } else { // all other types:
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].pos.chr, svs[i].pos.stop, id, svs[i].seq, svs[i].ref, "1/1",svs[i].copy_num,svs[i].recom).c_str());
                }
                svs2[i].print=false;
            } else { // het SVs!
                if (svs[i].type == 3) {
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].target.chr, svs[i].target.start, id, svs[i].seq, svs[i].ref, "1|0",svs[i].copy_num,svs[i].recom).c_str());
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.stop, svs[i].type, svs[i].target.chr, svs[i].target.stop, id, svs[i].seq, svs[i].ref, "1|0",svs[i].copy_num,svs[i].recom).c_str());
                } else { // all other types:
                    fprintf(file2, "%s", print_vcf_sv(svs[i].pos.chr, svs[i].pos.start, svs[i].type, svs[i].pos.chr, svs[i].pos.stop, id, svs[i].seq, svs[i].ref, "1|0",svs[i].copy_num,svs[i].recom).c_str());
                }
                id++;
            }
        }
        ...
        apply_mutations(genome, ori_genome,svs2,par.dup_max_amp,false);
        apply_mutations(genome2,ori_genome, svs2,par.dup_max_amp,true);	// apply changes to the genome (e.g. insertions).

        for (size_t i = 0; i < svs2.size(); i++) {
            if (svs2[i].print) {
                if (svs2[i].type == 3) {
                    fprintf(file2, "%s", print_vcf_sv(svs2[i].pos.chr, svs2[i].pos.start, svs2[i].type, svs2[i].target.chr, svs2[i].target.start, id, svs2[i].seq, svs2[i].ref, "0|1",svs2[i].copy_num,svs2[i].recom).c_str());
                    fprintf(file2, "%s", print_vcf_sv(svs2[i].pos.chr, svs2[i].pos.stop, svs2[i].type, svs2[i].target.chr, svs2[i].target.stop, id, svs2[i].seq, svs2[i].ref, "0|1",svs2[i].copy_num,svs2[i].recom).c_str());
                } else { // all other types:
                    fprintf(file2, "%s", print_vcf_sv(svs2[i].pos.chr, svs2[i].pos.start, svs2[i].type, svs2[i].pos.chr, svs2[i].pos.stop, id, svs2[i].seq, svs2[i].ref, "0|1",svs2[i].copy_num,svs2[i].recom).c_str());
                }
                id++;
            }
        ...
        std::cout << "write genome2" << std::endl;
        write_fasta(output_prefix, genome, true,"_m");
        std::cout << "write genome recom 2" << std::endl;
        write_fasta(output_prefix, genome2, true,"_mrecom");
        std::cout << "write SV" << std::endl;
        //	write_sv(output_prefix, svs2);
        write_sv(output_prefix, svs, false);
        write_sv(output_prefix, svs2, true);
    } else {
        write_sv(output_prefix, svs, false);
